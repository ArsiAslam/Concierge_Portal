...
import { ChevronLeft, ChevronRight, Building2, LogOut } from 'lucide-react'
...
export function Sidebar({ user }: SidebarProps) {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className={cn('flex h-16 items-center border-b border-white/10', collapsed ? 'justify-center px-2' : 'px-5 gap-3')}>
        <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-brand-600">
          <Building2 className="h-4 w-4 text-white" />
        </div>
        {!collapsed && (
          <div>
            <p className="text-sm font-bold text-white leading-none">Concierge</p>
            <p className="text-[10px] text-brand-300 uppercase tracking-wider">Portal</p>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
        {NAV_SECTIONS.map((section, si) => {
          const visibleItems = section.items.filter((item) => canSee(item.requiredPermission))
        ...
        {!collapsed ? (
          <div className="flex items-center gap-3">
            ...
            <form action={logoutAction}>
              <button type="submit" className="text-white/50 hover:text-white transition-colors" title="Sign out">
