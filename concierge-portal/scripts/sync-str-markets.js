// ---------------------------
// sync-str-markets.js
// Run locally:  node scripts/sync-str-markets.js
...
async function fetchRows() {
  const url = `${SUPABASE_URL}/rest/v1/str_markets?select=id,market`;

  log("Fetching STR markets from Supabase...");
  ...
  log(`Fetched ${rows.length} STR markets from Supabase`);
  return rows;
}
...
async function updateRow(id, market, agentCount) {
  const url = `${SUPABASE_URL}/rest/v1/str_markets?id=eq.${id}`;
...
async function run() {
  ...
  if (!SUPABASE_URL || !SUPABASE_KEY) {
    console.error(
      "[FATAL] Missing environment variables.\n" +
        ...
        "  Local:  node --env-file=.env scripts/sync-str-markets.js\n" +
  ...
  log(`Starting sync for ${rows.length} STR markets...\n`);
